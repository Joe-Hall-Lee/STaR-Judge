...
def build_dataset_UF(data_path, tokenizer, split='train', size=None, mode='', model_name=''):
    ...
    ds = ds.filter(
        lambda example: example['conv_A_rating'] != example['conv_B_rating'], num_proc=30)
    ...
    def formatting_func(example):
        ...
        if 'summarize' in example['source']:
            chosen_messages[0]['content'] = 'Generate one-sentence summary for the following post: ' + \
                chosen_messages[0]['content'].strip()
            rejected_messages[0]['content'] = 'Generate one-sentence summary for the following post: ' + \
...
def build_dataset_SK(data_path, tokenizer, split='train', size=None, model_name=''):
    ...
    def formatting_func(example):
        ...
        if 'GRM' in model_name:
            ...
            prompt_template = tokenizer.apply_chat_template(
                [{"content": prompt, "role": "user"}], tokenize=False, add_generation_prompt=True)
            tokens_prompt = tokenizer.encode_plus(
                prompt_template, **kwargs)['input_ids'][0]
            label_chosen = tokens_chosen["input_ids"][0].clone()
            label_chosen[:len(tokens_prompt)] = -100
