...
def convert_json_to_target_format(input_file, output_file, et_file_path):
    ...
    with open(input_file, 'r') as file:
        ...
        for item in data["arena"]:
            if item["result"]["orig"]["is_correct"] == True:
                ...
                output = item["result"]["orig"]["completion"]
            elif item["result"]["swap"]["is_correct"] == True:
                item["response1"], item["response2"] = item["response2"], item["response1"]
                if item["result"]["swap"]["prediction"] == 1:
                    chosen, rejected = "Output (a)", "Output (b)"
                elif item["result"]["swap"]["prediction"] == 2:
                    chosen, rejected = "Output (b)", "Output (a)"
                output = item["result"]["swap"]["completion"]
            else:
                continue
...
Given that {chosen} is better than {rejected}, please provide the rationale and end with "Therefore, {chosen} is better.":""",
            ...
            et_object = {
                "syetem": """You are a helpful assistant in evaluating the quality of the outputs for a given instruction. Your goal is to select the best output for the given instruction.""",
...
input_file_path = '../result/Llama-3.2-3B-Instruct/arena.json'
cot_file_path = '../data/arena_rationale.json'
et_file_path = '../data/arena_et_cot.json'

convert_json_to_target_format(input_file_path, cot_file_path, et_file_path)
